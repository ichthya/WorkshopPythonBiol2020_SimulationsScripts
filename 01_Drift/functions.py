...
#		print k, pops[k]
		for i in [50, 100,150,200,250,300,350,400,450]:
			line = list(map(str, [k, i] + list(pops[k][i])	))
			#print line
			out.write(','.join(line) + '\n')
...
def loadPop(inputFile, gzip = False):
	if gzip:
		import gzip
		pop = np.array([list(map(float, i.strip().split() )) for i in gzip.open(inputFile)])
	else:
		pop = np.array([list(map(float, i.strip().split() )) for i in open(inputFile)])
	return pop
